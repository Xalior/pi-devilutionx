//
// kernel.cpp — brings the board up, then hands control to DevilutionX.
//
// DevilutionX is an ordinary command-line program: it expects a working
// standard library, a filesystem holding the Diablo data, and an SDL2
...
// `diablo.ini` into the card's root would each silently overwrite the
// other's.
//
...
// DevilutionX's entry point. It is main() in the upstream source
// (Source/main.cpp); the build renames it for that one translation unit,
// because main() here belongs to the Circle kernel. Declared extern "C"
// because upstream declares it that way, and the signature must match.
extern "C" int devilutionx_main(int argc, char **argv);
...
static const char From[] = "devilutionx";

// The game's command line.
//
// argv[0] is an absolute path inside the game's own directory, for the sake
// of anything that reports the program's name.
//
// The three directory switches are upstream's own. They are what keeps the
// settings file, the save games and the search for DIABDAT.MPQ inside this
// game's directory without a single line of upstream being changed:
//
//   --data-dir     where diabdat.mpq / spawn.mpq are looked for
//   --save-dir     where save games are written
//   --config-dir   where diablo.ini is read and written
//
...
// without the card or the build changing — `--spawn` to force shareware
// mode, say, or `--verbose` for a noisier log.
static const char *DevilutionXArgv[] = {
    RAPI_GAME_DIR "/devilutionx",
    "--data-dir", RAPI_GAME_DIR,
    "--save-dir", RAPI_GAME_DIR,
    "--config-dir", RAPI_GAME_DIR,
};
...
// terminating null.
static const char *s_FinalArgv[sizeof(DevilutionXArgv) / sizeof(DevilutionXArgv[0])
...
void CSplitCores::Run(unsigned nCore)
{
    ...
    case 1:
        ...
        s_AppResult = devilutionx_main(s_FinalArgc, const_cast<char **>(s_FinalArgv));
...
// The second constructor array, placed by devilutionx-defaults.ld. Declared
// as data and taken by address: these are linker-defined symbols, so their
...
boolean CKernel::Initialize(void)
{
    ...
    // is plausible. DevilutionX stamps save games with it.
...
{
    m_Logger.Write(From, LogNotice, "starting DevilutionX");
    ...
    // the library about the display. Every SDL answer DevilutionX gets — the
    ...
    //
    // 640x480 is DevilutionX's own default resolution (DEFAULT_WIDTH and
    // DEFAULT_HEIGHT in Source/options.cpp), and the size its `diablo.ini`
    // is written with when there is no file yet. The game draws into an
    // 8-bit surface of this size and uploads it to one streaming texture,
    // which the library then scales to the panel.
    //
    ...
    s_FinalArgc = DefaultsBuildArgv(DevilutionXArgv,
    ...
    // card carries several games, and two of them writing a `diablo.ini`
    ...
    // simply look where the baked switches point it instead.
    ...
    else
    {
        ...
        res = devilutionx_main(s_FinalArgc, const_cast<char **>(s_FinalArgv));
    }
    ...
    m_Logger.Write(From, LogNotice, "DevilutionX exited with %d — parked", res);
    for (;;)
    {
