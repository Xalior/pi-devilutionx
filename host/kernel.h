//
// kernel.h — the Circle kernel that hosts DevilutionX.
//
// Device ownership: this kernel brings up interrupts, the timer, a serial
// console carrying stdio, the SD card (FatFs, holding the MPQ archives, the
// settings file and the save games) and the cooperative scheduler. Video,
...
//   core 1   the APPLICATION core. DevilutionX, alone: the game loop and
...
private:
    // Runs DevilutionX's own static constructors. The linker script keeps
    // them out of the array Circle's sysinit walks, because that walk happens
